...
# File paths
input_file_path = "data/merged.csv"  # Replace with your input file path
output_file_path = "data/processed.csv"  # Replace with your output file path
...
# Function to fetch the closing price with caching and delaying
def fetch_closing_price(ticker, target_date, max_retries=7, direction="backward", delay=1):
    """
    Fetches the closing price for a given ticker and date, with retries and caching.
    """
    ...
    while retry_count < max_retries:
        try:
            ...
            if not history.empty:
                closing_price = history['Close'].iloc[0]
                cache[cache_key] = (closing_price, retry_count)
                return closing_price, retry_count
            else:
                target_date += timedelta(days=delta)
                retry_count += 1
            ...
            target_date += timedelta(days=delta)
            retry_count += 1
        time.sleep(delay)  # Introduce delay to comply with rate limits
    
    cache[cache_key] = (None, retry_count)
    return None, retry_count
